...
function writeToTmpSass (fileName, fileContents) {
  fse.ensureDirSync(tempSassPath, { recursive: true })
  fs.writeFileSync(path.join(tempPath, '.gitignore'), '*')
  fs.writeFileSync(path.join(tempSassPath, fileName), fileContents)
}

function createSassImportFor (filePath) {
...
function sassExtensions () {
  ...
  fileContents += extensions.getFileSystemPaths('sass')
    .map(filePath => createSassImportFor(filePath))
    .join('\n')
  writeToTmpSass('_extensions.scss', fileContents)
}

function userApplicationSass () {
  const fileName = '_application.scss'
  const pathToUserSass = path.join(projectDir, 'app', 'assets', 'sass', 'application.scss')
  if (fs.existsSync(pathToUserSass)) {
    writeToTmpSass(fileName, createSassImportFor(pathToUserSass))
  } else {
    writeToTmpSass(fileName, '')
  }
}
