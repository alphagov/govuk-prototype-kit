...
async function getPluginsViewHandler (req, res) {
  ...
  var plugin = await getPluginForRequest(req)
  console.timeEnd("getPluginForRequest")

  if (!plugin){
    for (let newPlugin of newPlugins){
      if (newPlugin.packageName == req.query.package){
        plugin = newPlugin
        break
      }
    }
...
async function getPluginsHandler (req, res) {
  ...
  // add new plugins
  model.groupsOfPlugins[1].plugins.push(...newPlugins)

  model.showStatus = req.query.status
