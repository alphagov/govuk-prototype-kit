...
const { lookupPackageInfo, getInstalledPackages, getAvailablePackages } = require('./utils/packages')
...
function buildPluginData (data) {
  return data.map((pluginData) => {
    const {
      name: packageName,
      installed,
      installedLocally,
...
async function prepareForPluginPage () {
  ...
  installedOut.plugins = buildPluginData(await getInstalledPackages())
  installedOut.status = 'installed'
  availableOut.name = 'Available'
  availableOut.plugins = buildPluginData(await getAvailablePackages())
