...
const {
  ...
  getLatestPluginDetailsFromNpm, getPluginDetailsFromRef
...
function buildPluginData (pluginData) {
  ...
  return {
    ...
    installedVersion,
    installedLocally,
    installLink: installLink || `${contextPath}/plugins/install?package=${encodeURIComponent(packageName)}`,
    installCommand: installCommand || `npm install ${packageName}`,
    updateLink: updateLink || (installed && !installedLocally && latestVersion !== installedVersion ? `${contextPath}/plugins/update?package=${encodeURIComponent(packageName)}` : undefined),
    updateCommand: updateCommand || (latestVersion && `npm install ${packageName}@${latestVersion}`),
    uninstallLink: uninstallLink || (installed && !required ? `${contextPath}/plugins/uninstall?package=${encodeURIComponent(packageName)}${installedLocally ? `&version=${encodeURIComponent(localVersion)}` : ''}` : undefined),
    uninstallCommand: uninstallCommand || `npm uninstall ${packageName}`,
    releaseDateTime,
    releaseTimeSummary: releaseDateTime && getTimeSummary(new Date(releaseDateTime)),
    urlEncodedPackageName: encodeURIComponent((installedVersion ? 'installed:' : 'npm:') + packageName)
  }
}
...
function getCommand (mode, chosenPlugin) {
  ...
  if (version && installCommand && !fileSystemLocation) {
...
async function getModeRedirectForPackageRef (req, res, next) {
  const { mode, packageRef } = req.params
  const pluginDetails = await getPluginDetailsFromRef(packageRef)

  if (mode === 'uninstall') {
    return res.redirect([contextPath, 'plugins', `uninstall?package=${pluginDetails.packageName}`].join('/'))
  }
  if (mode === 'install') {
    return res.redirect([contextPath, 'plugins', `install${pluginDetails.legacyInstallQueryString}`].join('/'))
  }
  if (mode === 'update') {
    return res.redirect([contextPath, 'plugins', `update${pluginDetails.legacyInstallQueryString}`].join('/'))
  }
  res.send(`Unknown mode ${mode}`)
}
...
async function getPluginForRequest (req) {
  ...
  console.log(' ---- ')
  console.log('')
  console.log(packageName, version, mode)
  console.log('')
  console.log(' ---- ')
  ...
  if (packageName) {
    ...
    if (!chosenPlugin) {
      console.log('no chosen plugin found')
      return // chosen plugin will be invalid
    }
    console.log('chosen plugin found', chosenPlugin.packageName, chosenPlugin.version)
    console.log(chosenPlugin)
    if (version) {
      if (chosenPlugin.installedLocally) {
        chosenPlugin.version = chosenPlugin.installedVersion
      } else {
        const pluginDetailsFromFileSystemDependency = await getDetailsIfValidFileSystemDependency(version)
        if (pluginDetailsFromFileSystemDependency) {
          chosenPlugin.version = pluginDetailsFromFileSystemDependency.packageJson.version
          chosenPlugin.fileSystemLocation = pluginDetailsFromFileSystemDependency.fileSystemPath
        } else if (await isValidVersion(packageName, version)) {
...
async function getPluginsModeHandler (req, res) {
  ...
  const pluginsReturnLink = {
    href: ['', 'manage-prototype', 'plugins'].join('/') + (req.query.version ? '?version=' + encodeURIComponent(req.query.version) : ''),
    text: 'Back to plugin list'
  }
