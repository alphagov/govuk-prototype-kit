...
async function getHomeHandler (req, res) {
  ...
  const viewData = {
    ...
    kitUpgradeAvailable: kitPackage.latestVersion !== currentKitVersion,
...
function buildPluginData (pluginData) {
  ...
  return {
    ...
    uninstallLink: installed && !mandatory ? `${contextPath}/plugins/uninstall?package=${encodeURIComponent(packageName)}${installedLocally ? `&version=${encodeURIComponent(localVersion)}` : ''}` : undefined,
