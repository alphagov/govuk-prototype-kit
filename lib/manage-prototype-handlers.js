...
const { projectDir, packageDir, pluginsProcessingDir } = require('./utils/paths')
...
async function getPluginsProcessingConfigPath () {
  await fse.ensureDir(path.join(pluginsProcessingDir))
  return path.join(pluginsProcessingDir, 'plugins-processing-config.json')
}

async function loadPluginsProcessingConfig () {
  const pluginsProcessingConfig = await getPluginsProcessingConfigPath()
  return (await fse.pathExists(pluginsProcessingConfig)) ? await fse.readJson(pluginsProcessingConfig) : {}
}

async function savePluginsProcessingConfig (config) {
  const pluginsProcessingConfig = await getPluginsProcessingConfigPath()
  return fse.writeJson(pluginsProcessingConfig, config)
}
...
async function postPluginsStatusHandler (req, res) {
  const { mode } = req.params
  const processingConfig = await loadPluginsProcessingConfig()
  let status = 'processing'
  try {
    if (processingConfig[req.body.package]) {
  ...
  if (status === 'completed') {
    const processingConfig = await loadPluginsProcessingConfig()
    delete processingConfig[req.body.package]
    await savePluginsProcessingConfig(processingConfig)
  }
  res.json({ status })
}
...
async function postPluginsModeHandler (req, res) {
  ...
  // verb holds a list of verbs depending on the process, if the mode is invalid the set will not exist
  const verb = verbs[mode]
  ...
  if (status === 'processing') {
    processingConfig[req.body.package] = true
  } else {
    delete processingConfig[req.body.package]
  }
