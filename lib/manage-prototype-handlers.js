...
async function postPluginDetailsHandler (req, res) {
  const pointToDetailsPage = true
  const [pluginName, version] = req.body.pluginPackageName.split('@')
  if (pointToDetailsPage) {
    res.redirect(req.originalUrl + '/' + encodeURIComponent(pluginName) + (version ? `?version=${encodeURIComponent(version)}` : ''))
  } else {
    res.redirect(req.originalUrl + 's/install/' + `?package=${encodeURIComponent(pluginName)}` + (version ? `&version=${encodeURIComponent(version)}` : ''))
  }
}
...
async function getPluginDetailsHandler (req, res, next) {
  ...
  console.log(pluginDetails.packageName, req.params.packageName)
  if (pluginDetails.packageName !== req.params.packageName) {
    const redirectUrl = `${contextPath}/plugin/${encodeURIComponent(pluginDetails.packageName)}` + (req.query.version ? `?version=${encodeURIComponent(req.query.version)}` : '')
...
async function getPluginsModeHandler (req, res) {
  ...
  const pluginsReturnLink = {
    href: ['', 'manage-prototype', 'plugin', chosenPlugin.packageName].join('/') + (req.query.version ? '?version=' + encodeURIComponent(req.query.version) : ''),
