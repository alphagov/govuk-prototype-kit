...
async function postTemplatesInstallHandler (req, res) {
  ...
  const installLocation = path.join(projectDir, 'app', 'views', `${chosenUrl}.${(getFileExtensionForNunjucksFiles())}`)
...
const prototypeTimestamp = Date.now()

async function postPluginsStatusHandler (req, res) {
  const { mode } = req.params
  let status = 'processing'
  try {
    if (Date.now() < prototypeTimestamp + 4000) {
      // The prototype restarted less than 4 seconds ago
