...
const {
  ...
  waitForPackagesCache, getDependencyPackages
...
async function isValidVersion (packageName, version) {
  const { versions = [] } = await lookupPackageInfo(packageName)
  const isVersionValid = versions.includes(version)
  if (!isVersionValid) {
...
async function getPluginForRequest (req) {
  ...
  let chosenPlugin
  if (searchPackage) {
    chosenPlugin = buildPluginData(await lookupPackageInfo(searchPackage))
...
async function getPluginsModeHandler (req, res) {
  ...
  const dependentPlugins = (await getDependentPackages(chosenPlugin.packageName))
    // .filter(({ installed }) => installed)
    .map(buildPluginData)

  const dependencyPlugins = (await getDependencyPackages(chosenPlugin.packageName))
    // .filter(({ installed }) => !installed)
    .map(buildPluginData)
