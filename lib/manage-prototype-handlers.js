...
async function pluginCommandsAreLocked () {
  const pluginsProcessingConfig = await getPluginsProcessingConfigPath()
  const { locked } = (await fse.pathExists(pluginsProcessingConfig)) ? await fse.readJson(pluginsProcessingConfig) : {}
  return locked
}

async function lockPluginCommands () {
  const pluginsProcessingConfig = await getPluginsProcessingConfigPath()
  return fse.writeJson(pluginsProcessingConfig, { locked: true })
}

async function unlockPluginCommands () {
  const pluginsProcessingConfig = await getPluginsProcessingConfigPath()
  return fse.writeJson(pluginsProcessingConfig, { locked: false })
}
...
async function postPluginsModeHandler (req, res) {
  ...
  let status = 'processing'
  try {
    if (await pluginCommandsAreLocked()) {
