...
const { prototypeAppScripts, sortByObjectKey } = require('./utils')
...
async function isValidVersion (packageName, version) {
  ...
  const isVersionValid = versions.includes(version) || version === localVersion
  if (!isVersionValid) {
...
async function lookupLatestPackageVersion ([packageName, version]) {
  try {
...
async function updateLatestReleaseVersions () {
  ...
  await Promise.all(Object.entries(latestReleaseVersions).map(lookupLatestPackageVersion))
...
function getCommand (mode, chosenPlugin) {
  let { upgradeCommand, installCommand, uninstallCommand, version, dependencyPlugins, dependentPlugins } = chosenPlugin
  const dependants = dependentPlugins?.map(({ packageName }) => packageName).join(' ')
...
async function getPluginForRequest (req) {
  ...
  if (searchPackage) {
    chosenPlugin = buildPluginData(await lookupPackageInfo(searchPackage, version))
...
function modeIsComplete (mode, { installedVersion, latestVersion, version }) {
  switch (mode) {
    ...
    case 'install':
      return version ? installedVersion === version : !!installedVersion
    case 'uninstall':
...
async function getPluginsModeHandler (req, res) {
  ...
  if (chosenPlugin?.dependentPlugins?.length) {
    pluginHeading = `Other plugins need ${fullPluginName}`
  } else if (chosenPlugin?.dependencyPlugins?.length) {
    pluginHeading = `${fullPluginName} needs other plugins`
  }
