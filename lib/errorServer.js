...
function runErrorServer (error) {
  ...
  const requestListener = function (req, res) {
    ...
    try {
      ...
      res.end(nunjucksAppEnv.render('views/error-handling/server-error', errorView(error)))
    } catch (ignoreThisError) {
      console.log(JSON.stringify({ ignoreThisError }, null, 2))
      fileContentsParts.push('<h1 class="govuk-heading-l">There is an error using the default markup</h1>')
