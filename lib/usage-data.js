...
exports.collectDataUsage = async function () {
  // Get usageDataConfig from file, if exists
  const usageDataConfig = exports.getUsageDataConfig()

  if (usageDataConfig.collectUsageData === undefined) {
    // No recorded answer, so ask for permission
    const permissionGranted = await exports.askForUsageDataPermission()
    usageDataConfig.collectUsageData = permissionGranted
    exports.setUsageDataConfig(usageDataConfig)

    if (permissionGranted) {
      exports.startTracking(usageDataConfig)
    }
  } else {
