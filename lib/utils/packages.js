...
async function refreshPackageInfo (packageName) {
  ...
  packagesCache[packageName] = Object.fromEntries(Object.entries(packageInfo).filter(([key, value]) => value !== undefined))
}
...
async function getInstalledPackages () {
  return Object.values(packagesCache).filter(({ installed }) => installed)
}

async function getAvailablePackages () {
  return Object.values(packagesCache).filter(({
    available,
    installed
  }) => available && !installed)
}
