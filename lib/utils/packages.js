...
const { getConfigForPackage } = require('./requestHttps')
...
async function refreshPackageInfo (packageName, version) {
  ...
  const pluginDependencies = pluginConfig?.dependsUpon
...
async function getDependentPackages (packageName) {
  ...
  await waitForPackagesCache()
  return Object.values(packagesCache)
    .filter(({ pluginDependencies }) => pluginDependencies?.some((pluginDependency) => pluginDependency.packageName === packageName))
}
...
async function getDependencyPackages (packageName, version) {
  ...
  return pkg?.pluginDependencies ? await Promise.all(pkg.pluginDependencies.map((pluginDependency) => lookupPackageInfo(pluginDependency.packageName))) : []
}
