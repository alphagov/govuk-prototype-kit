...
function sassMatcher (error) {
  ...
  return {
    ...
    unstyledErrorStack: unstyle(error.stack),
...
function getErrorDetails (cwd, error) {
  ...
  return {
    ...
    normalisedFilePath,
    line,
    errorStack: unstyledErrorStack || error.stack,
...
const getErrorModel = function (error) {
  ...
  const viewObject = {
    pluginConfig: {
      scripts: [
        {
          src: '/plugin-assets/%40nowprototypeit%2Fgovuk/lib/assets/javascripts/kit.js'
        }
      ]
    }
  }
