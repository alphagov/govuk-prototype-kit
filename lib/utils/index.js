...
// Find the first port in the given range (inclusive) that is not in use
async function findAPortNotInUse (fromPort, toPort) {
  for (let port = fromPort; port <= toPort; port++) {
    if (await isPortAvailable(port)) {
      return port
    }
  }
  throw new Error(`Could not find an available port between ${fromPort} and ${toPort}`)
}
...
async function findAvailablePort () {
  ...
  const availablePort = await findAPortNotInUse(port, port + 50)
