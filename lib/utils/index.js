...
function renderPath (path, res, next) {
  res.myTestData.log('renderPath-start')
  const model = {}
  if (path === 'index') {
  ...
  res.render(path, model, (error, html) => {
    res.myTestData.log('renderPath-render')
    if (!error) {
      // Success - send the response
      res.set({ 'Content-type': 'text/html; charset=utf-8' })
      res.end(html)
      res.myTestData.log('renderPath-end')
      return
    }
    if (!error.message.startsWith('template not found')) {
      // We got an error other than template not found - call next with the error
      res.myTestData.log('renderPath-404')
      next(error)
      return
    }
...
function matchRoutes (req, res, next) {
  res.myTestData.log('matchRoutes')
