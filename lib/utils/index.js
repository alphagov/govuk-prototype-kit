...
async function renderPath (urlPath, res, next) {
  ...
  if (await fileExists(path.join(projectDir, 'app', 'views', urlPath + '.html'))) {
    urlPath += '.html'
  }
...
async function matchRoutes (req, res, next) {
  ...
  // look for the file on the file system in app/views
  // see whether it exists as .njk or .html
  // feed the actual file name into renderPath
