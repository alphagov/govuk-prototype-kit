...
async function renderPath (urlPath, res, next) {
  ...
  const htmlExists = await fileExists(path.join(appViewsDir, urlPath + '.html'))
  const njkExists  = await fileExists(path.join(appViewsDir, urlPath + '.njk'))
  const preferHtml = !config.useNjkExtensions

  console.table({ htmlExists, njkExists, preferHtml })

  if (htmlExists) {
    if (preferHtml || !njkExists) {
      urlPath += '.html'
    }
  }
