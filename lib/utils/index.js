...
const globals = require('../globals/api')
...
// Require core and custom globals, merges to one object
// and then add the methods to Nunjucks environment
function addNunjucksGlobals (env) {
  globals.setEnvironment(env)
  const additionalGlobals = []
  const globalsPath = path.join(appDir, 'globals.js')
  if (fs.existsSync(globalsPath)) {
    additionalGlobals.push(globalsPath)
  }
  const globalFiles = plugins.getFileSystemPaths('nunjucksGlobals').concat(additionalGlobals)
