...
const { appDir, projectDir, portTmpFile } = require('./paths')
...
function findAvailablePort (app, callback) {
  let port = null

  // When the server starts, we store the port in port.tmp so it tries to restart
  // on the same port
  try {
    port = Number(fs.readFileSync(portTmpFile))
  } catch (e) {
    port = config.port
  }
    ...
    if (port === availablePort) {
      ...
      inquirer.prompt([{
        ...
        if (answers.changePort) {
          // User answers yes
          port = availablePort
          fs.writeFileSync(path.join(portTmpFile), port.toString())
