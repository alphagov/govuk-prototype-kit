  ...
  try {
    ...
    try {
      ...
      if (getConfig().showPrereleases) {
        version = Object.keys(registerEntry['dist-tags']).map(tag => ({
          tag,
        ...
        version = registerEntry['dist-tags'].latest
      }
      result = await self.getPluginDetailsFromNpm(packageName, version)
    } catch (e) {
      if (e.statusCode === 404) {
        return doesntExistResponse
      }
