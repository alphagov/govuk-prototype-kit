...
function addExpectedLinks (config) {
  const pluginDetails = ['', 'manage-prototype', 'plugin', config.internalRef].map(encodeURIComponent).join('/')
  return {
    ...config,
    links: {
...
describe.only('Package Details', () => {
  ...
  describe('getLatestPluginDetailsFromNpm', () => {
    beforeEach(() => {
      testScope.mockedResult = { packageName: 'example-plugin' }
  ...
  describe('getPluginDetailsFromFileSystem', () => {
    beforeEach(() => {
      testScope.fakeDocsDir = '/Users/my.fake.user/Documents'
