...
function clean () {
  del.sync([backupNunjucksDir, shadowNunjucksDir])
...
function createBackupNunjucks () {
  ...
  function backupFilesWithExtension (fileExtension, backupExtension) {
    ...
    filesToBackup.forEach(fileName => {
      ...
      const includeMethod = fileName.startsWith('layouts/') || fileName.includes('/layouts/') ? 'extends' : 'include'
