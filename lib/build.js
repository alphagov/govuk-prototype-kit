...
const { recursiveDirectoryContentsSync, verboseLog } = require('./utils')
const { startPerformanceTimer, endPerformanceTimer } = require('./utils/performance')

let kitVersion

try {
  kitVersion = fse.readJsonSync(path.join(packageDir, 'package.json')).version
} catch (e) {
  kitVersion = 'unknown'
}
...
function generateAssetsSync ({ verbose } = {}) {
  verboseLog('Generating assets, kit version is:', kitVersion)
