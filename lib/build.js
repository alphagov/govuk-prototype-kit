...
const { pluginVersionSatisfies } = require('./plugins/packages')
...
function cleanShadowNunjucks () {
  fse.emptyDirSync(shadowNunjucksDir)
}
function cleanBackupNunjucks () {
  fse.emptyDirSync(backupNunjucksDir)
}
...
const sassFunctions = {
  'plugin-version-satisfies($plugin-name, $semver-range)' (args) {
    const pluginName = args[0].assertString('plugin-name').text
    const semverRange = args[1].assertString('semver-range').text

    const result = pluginVersionSatisfies(pluginName, semverRange)

    return result ? sass.sassTrue : sass.sassFalse
  }
}
