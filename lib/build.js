...
import { projectDir, appSassDir, libSassDir, tmpDir, tmpSassDir, publicCssDir, shadowNunjucksDir, backupNunjucksDir, appViewsDir } from "./utils/paths.js";
import { recursiveDirectoryContentsSync } from "./utils/index.js";
...
function autoImportComponentsSync() {
    const timer = startPerformanceTimer();
    const includeString = plugins.getByType('nunjucksMacros').map(({ item: { macroName, importFrom } }) => `{% from "${importFrom}" import ${macroName} %}`).join('\n');
...
export { generateAssetsSync };
export { generateCssSync };
export { proxyUserSassIfItExists };
export default {
    generateAssetsSync,
    generateCssSync,
