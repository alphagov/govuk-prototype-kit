  ...
  return new Promise((resolve, reject) => {
    child.on('close', (code) => {
      if (code === 0) {
        ...
        const error = new Error(`Exit code was ${code} from command ${command}`)
        error.code = code
  ...
  return new Promise((resolve, reject) => {
    child.on('close', (code) => {
      if (code === 0) {
        ...
        reject(new Error(`Exit code was ${code}, command was: ${command}, arguments were ${JSON.stringify(args)}`))
      }
    })
  })
}
