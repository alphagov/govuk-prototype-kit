...
const matchAgainstOldVersions = async (filePath) => {
  ...
  const userFile = await fs.readFile(userPath, 'utf8').catch(handleNotFound(false))

  if (userFile === false) {
    return
  }

  const listOfKnownReleaseVersions = await fs.readdir(oldVersionsDir)
    .then(files => Promise.all(
      files.map(fileName => fs.readFile(path.join(oldVersionsDir, fileName), 'utf8'))
    ))

  const output = userFile && listOfKnownReleaseVersions.filter(knownVersion => knownVersion === userFile).length > 0
