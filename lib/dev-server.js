...
const wrapper = require('./dev-server-wrapper')
...
function runNodemon(port) {
  ...
  const onRestart = () => {
    wrapper.triggerStopErrorServer()
    console.log('Restarting kit...')
  }

  // Warn about npm install on crash
  function onCrash() {
    console.log('spinning up error server')
    wrapper.spinUpErrorServer()
  ...
  return nodemon({
    ...
    .on('readable', function () {
      ...
      this.stderr.on('data', function (chunk) {
        ...
        wrapper.errorStore.store(chunkAsString)
        console.log('----')
