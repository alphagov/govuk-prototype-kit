...
// local dependencies
const { generateAssetsSync, generateCssSync, proxyUserSassIfItExists, rebuildNunjucksSync } = require('./build')
...
function watchSass (sassPath) {
  ...
  chokidar.watch(sassPath, standardChokidarOptions)
    .on('add', proxyAndGenerateCssSync)
    .on('unlink', proxyAndGenerateCssSync)
    .on('all', generateCssSync)
}

function watchNunjucks (appViewsPath) {
  if (!fse.existsSync(appViewsPath)) return
  const listener = () => {
    rebuildNunjucksSync()
  }
  chokidar.watch(appViewsPath, standardChokidarOptions)
    .on('add', listener)
    .on('unlink', listener)
}
