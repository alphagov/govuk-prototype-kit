...
const {
  ...
  runPluginMode,
  getCommandStatus, getPluginsModeHandler
...
router.use((err, req, res, next) => {
  if (err.status === 404) {
    ...
    res.status(err.status || 500).render('views/error-handling/server-error.njk', { error: { message: err.message, errorStack: err.stack } })
  }
})
