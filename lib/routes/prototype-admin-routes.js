...
const getCommand = (mode, chosenPlugin) => {
  switch (mode) {
...
router.post('/plugins/:mode', async (req, res) => {
  try {
    ...
    exec(command, { cwd: projectDir }).finally(() => {
      ...
      // force the application to stop as nodemon restart does not always work on Windows when running acceptance tests
