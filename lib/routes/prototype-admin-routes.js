...
router.get('/templates/view', (req, res) => {
  const model = {
    extensionConfig: extensions.getAppConfig({ scripts: prototypeAppScripts }),
...
router.get('/plugins', async (req, res) => {
  ...
  if (isSuccessResult) {
    ...
    chokidar.watch(nodeModulesPath, {
      ignoreInitial: true,
      awaitWriteFinish: true,
      depth: 4
    }).on('change', () => setTimeout(renderPluginsPage, 1000))
  } else {
    setImmediate(renderPluginsPage)
  }
})
...
router.post('/plugins/install', async (req, res) => {
  ...
  try {
    const command = req.query.mode === 'upgrade' ? chosenPlugin.upgradeCommand : chosenPlugin.installCommand
