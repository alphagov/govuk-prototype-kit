...
redirectingRouter.use((req, res, next) => {
  res.redirect(req.originalUrl.replace('/prototype-admin', contextPath))
})
...
router.get('/', async (req, res) => {
  ...
  res.render(getManagementView('index.njk'), {
    ...
    tasks: [
      {
        done: config.serviceName !== 'Service name goes here',
...
async function getPluginDetails () {
  ...
  const all = await Promise.all(installed.concat(available)
    ...
    .map(async (packageDetails) => {
      ...
      pack.installLink = `${contextPath}/plugin/install?package=${encodeURIComponent(pack.packageName)}&mode=install`
      if (pack.installedVersion && pack.installedVersion !== pack.latestVersion) {
        pack.upgradeLink = `${contextPath}/plugin/install?package=${encodeURIComponent(pack.packageName)}&mode=upgrade`
      }
      return pack
    }))
  return all
...
router.get('/plugins', async (req, res) => {
  ...
  const groupsOfPlugins = await prepareForPluginPage()
...
router.get('/plugins/install', async (req, res) => {
  ...
  const chosenPlugin = (await getPluginDetails())
    .filter((packageDetails) => {
      ...
      return packageName === searchPackage
    })[0]
  if (!chosenPlugin) {
    res.status(404).send('Plugin not found')
    return
  }
  res.render(getManagementView('plugin-install.njk'), {
    currentPage: pageName,
    links: managementLinks,
    chosenPlugin
  })
