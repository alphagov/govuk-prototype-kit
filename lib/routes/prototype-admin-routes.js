  ...
  const all = await Promise.all(installed.concat(available)
    ...
    .map(async (packageDetails) => {
      ...
      Object.assign(pack, extensions.preparePackageNameForDisplay(pack.packageName), {
        ...
        installCommand: `npm i ${pack.packageName}@${pack.latestVersion}`,
...
router.get('/plugins/install', async (req, res) => {
  ...
  res.render(getManagementView('plugin-install-or-uninstall.njk'), {
    ...
    links: managementLinks,
    chosenPlugin,
    command: chosenPlugin.installCommand,
    verb: {
...
router.post('/plugins/install', async (req, res) => {
  ...
  res.redirect(`${contextPath}/plugins?success=${encodeURIComponent(chosenPlugin.packageName)}&mode=${encodeURIComponent(req.query.mode)}`)
})
...
router.post('/plugins/uninstall', async (req, res) => {
  ...
  res.redirect(`${contextPath}/plugins?success=${encodeURIComponent(chosenPlugin.packageName)}&mode=uninstall`)
})
