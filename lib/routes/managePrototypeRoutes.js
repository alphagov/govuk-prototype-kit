...
const routerConfig = { useStandardMiddleware: false }
const requests = require('../routes/api').external
const router = requests.setupRouter(contextPath, routerConfig)
const redirectingRouter = requests.setupRouter('/manage-prototype', routerConfig)
...
const latestReleaseVersions = {
  'govuk-prototype-kit': currentKitVersion,
  '@govuk-prototype-kit/step-by-step': 0,
  'govuk-frontend': 0,
  '@hmcts/frontend': 0,
  'hmrc-frontend': 0,
  jquery: 0
...
router.get('/clear-data', async (req, res) => {
  res.render(getManagementView('clear-data.njk'), await requests.generateStandardModel())
})

router.post('/clear-data', async (req, res) => {
  req.session.data = {}
  res.render(getManagementView('clear-data-success.njk'), await requests.generateStandardModel())
})
...
router.get('/', async (req, res) => {
  ...
  const currentHomepage = await fse.readFile(path.join(projectDir, 'app', 'views', 'index.html'), 'utf8')

  res.render(getManagementView('index.njk'), await requests.mergeWithStandardModel({
    ...
    tasks: [
      {
        done: getConfig().serviceName !== 'Service name goes here',
        html: 'Change the service name in the file <strong>app/config.json</strong>'
      }, {
        done: originalHomepage !== currentHomepage,
...
router.get('/templates', async (req, res) => {
  ...
  res.render(getManagementView('templates.njk'), await requests.mergeWithStandardModel({
    currentPage: pageName,
    links: managementLinks,
...
router.get('/templates/view', async (req, res) => {
  const model = await requests.mergeWithStandardModel({
    extensionConfig: {
...
router.get('/templates/install', async (req, res) => {
  const templateConfig = locateTemplateConfig(req)

  if (templateConfig) {
    res.render(getManagementView('template-install.njk'), await requests.mergeWithStandardModel({
...
router.get('/templates/post-install', async (req, res) => {
  const pageName = 'Templates'
  const chosenUrl = req.query['chosen-url']

  res.render(getManagementView('template-post-install.njk'), await requests.mergeWithStandardModel({
    currentPage: pageName,
    links: managementLinks,
    url: chosenUrl,
...
router.get('/plugins/install', async (req, res) => {
  ...
  res.render(getManagementView('plugin-install-or-uninstall.njk'), await requests.mergeWithStandardModel({
    currentPage: pageName,
    currentUrl: req.originalUrl,
    links: managementLinks,
    chosenPlugin,
...
router.post('/plugins/install', async (req, res) => {
  const chosenPlugin = await getPluginForRequest(req)
  res.render(getManagementView('plugin-install-process.njk'), await requests.mergeWithStandardModel({
    chosenPlugin,
    verb: {
