...
const mergeWithCoreModel = async (model) => ({ ...(await generateModel()), ...model })
...
router.get('/clear-data', async (req, res) => {
  res.render(getManagementView('clear-data.njk'), await generateModel())
})

router.post('/clear-data', async (req, res) => {
  req.session.data = {}
  res.render(getManagementView('clear-data-success.njk'), await generateModel())
})
...
router.get('/', async (req, res) => {
  ...
  res.render(getManagementView('index.njk'), await mergeWithCoreModel({
...
router.get('/templates', async (req, res) => {
  ...
  res.render(getManagementView('templates.njk'), await mergeWithCoreModel({
    currentPage: pageName,
    links: managementLinks,
...
router.get('/templates/view', async (req, res) => {
  const model = await mergeWithCoreModel({
    extensionConfig: {
...
router.get('/templates/install', async (req, res) => {
  ...
  if (templateConfig) {
    res.render(getManagementView('template-install.njk'), await mergeWithCoreModel({
...
router.get('/templates/post-install', async (req, res) => {
  ...
  res.render(getManagementView('template-post-install.njk'), await mergeWithCoreModel({
    currentPage: pageName,
    links: managementLinks,
    url: chosenUrl,
...
router.get('/plugins/install', async (req, res) => {
  ...
  res.render(getManagementView('plugin-install-or-uninstall.njk'), await mergeWithCoreModel({
    currentPage: pageName,
    currentUrl: req.originalUrl,
    links: managementLinks,
    chosenPlugin,
...
router.post('/plugins/install', async (req, res) => {
  const chosenPlugin = await getPluginForRequest(req)
  res.render(getManagementView('plugin-install-process.njk'), await mergeWithCoreModel({
    chosenPlugin,
    verb: {
