window.GOVUKPrototypeKit.documentReady(() => {
  const params = new URLSearchParams(window.location.search)
  const packageName = params.get('package')
  const version = params.get('version')
  const mode = params.get('mode') || window.location.pathname.split('/').pop()
  let requestTimeoutId
  let timedOut = false
  const timeout = 30 * 1000
  const delayToAllowRequestsToComplete = 10 * 1000
  let finalTryTimestamp
  let actionTimeoutId
  const token = document.querySelector('meta[name="csrf-token"]').getAttribute('content')

  const show = (id) => {
    const element = document.getElementById(id)
    element.hidden = false
  }

  const hide = (id) => {
    const element = document.getElementById(id)
    element.hidden = true
  }

  const log = (status) => {
    window.console.info(`GOV.UK Prototype Kit - ${status} ${mode} of ${packageName}`)
  }

  const showCompleteStatus = () => {
    clearTimeout(actionTimeoutId)
    log('Successful')
    hide('panel-processing')
    show('panel-complete')
    show('instructions-complete')
  }

  const showErrorStatus = () => {
    log('Failed')
    hide('panel-processing')
    show('panel-error')
  }

  const postRequest = (url) => fetch(url, {
    method: 'POST',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
      'CSRF-Token': token
    },
    body: JSON.stringify({
      package: packageName,
      version
    })
  })

  const pollStatus = () => {
    log('Processing')
    if (requestTimeoutId) {
      clearTimeout(requestTimeoutId)
    }
    if (timedOut) {
      showErrorStatus()
    } else {
      // Be aware that changing this path for monitoring the status of a plugin will affect the
      // kit upgrade process as the browser request and server route would be out of sync.
      finalTryTimestamp = Date.now() + delayToAllowRequestsToComplete
      postRequest(`/manage-prototype/plugins/${mode}/status`)
        .then(response => response.json())
        .then(data => {
          requestTimeoutId = setTimeout(() => {
            if (data.status === 'error') {
              clearTimeout(actionTimeoutId)
              showErrorStatus()
            } else if (data.status === 'completed') {
              showCompleteStatus()
            } else {
              // poll status again if prototype hasn't restarted
              pollStatus()
            }
          }, 1000)
        })
        .catch(() => {
          requestTimeoutId = setTimeout(pollStatus, 1000)
        })
    }
  }

  const performAction = (event) => {
    log('Starting')

    if (event) {
      event.preventDefault()
      hide('plugin-action-confirmation')
    }

    show('panel-processing')

    if (requestTimeoutId) {
      clearTimeout(requestTimeoutId)
    }
    if (timedOut) {
      showErrorStatus()
    } else {
      finalTryTimestamp = Date.now() + delayToAllowRequestsToComplete
      postRequest(`/manage-prototype/plugins/${mode}`)
        .then(response => response.json())
        .then(data => {
          switch (data.status) {
            case 'completed': {
              // Command has already been run
              showCompleteStatus()
              break
            }
            case 'locked': {
              // Command cannot be run while a concurrent plugin is processed
              requestTimeoutId = setTimeout(performAction, 2000)
              break
            }
            case 'processing': {
              pollStatus()
              break
            }
            default: {
              // Default to error
              showErrorStatus()
            }
          }
        })
        .catch(() => {
          // Command cannot be run while a concurrent plugin is processed
          requestTimeoutId = setTimeout(performAction, 2000)
        })
    }
  }

  hide('panel-manual-instructions')
  const actionButton = document.getElementById('plugin-action-button')

  const start = (event) => {
    actionTimeoutId = setTimeout(() => {
      if (finalTryTimestamp > Date.now()) {
        timedOut = true
      } else {
        setTimeout(() => {
          timedOut = true
        }, delayToAllowRequestsToComplete)
      }
    }, timeout)
    performAction(event)
  }

  if (actionButton) {
    actionButton.addEventListener('click', start)
  } else {
    start()
  }
})
