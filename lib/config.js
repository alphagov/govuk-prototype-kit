...
const getConfig = () => {
  ...
  const overrideOrDefault = (configName, envName, processor, defaultValue) => {
    ...
    if (environmentValue !== undefined) {
      console.log(`${envName} exists on environment`)
      config[configName] = processor(environmentValue)
      console.log(`Before processing ${environmentValue} after processing ${processor(environmentValue)}`)
    } else if (defaultValue !== undefined && config[configName] === undefined) {
      config[configName] = defaultValue
    }
  }

  console.log('reading config', process.env.NODE_ENV, process.env.USE_HTTPS)
  ...
  config.isSecure = !!(config.isProduction && config.useHttps)
