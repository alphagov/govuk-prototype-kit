...
const passwordVarsPath = path.join(appDir, 'password-vars.json')
...
function getConfig (config, swallowError = true) {
  ...
  if (process.env.PASSWORD) {
    passwords.push(process.env.PASSWORD)
  }

  if (fse.pathExistsSync(passwordVarsPath)) {
    const passwordVars = fse.readJsonSync(passwordVarsPath)
    if (Array.isArray(passwordVars)) {
      passwordVars
        .filter(passwordVar => process.env[passwordVar])
        .forEach(passwordVar => passwords.push(process.env[passwordVar]))
    }
  }
