  ...
  req.on('end', () => {
    ...
    const proxyReq = http.request({
      ...
      proxyRes.on('end', () => {
        ...
        // The error server doesn't always set a content type - default to HTML
        // so the dev server injects its live reload client into those pages
        const contentType = proxyRes.headers['content-type'] || 'text/html'
        res.setHeader('content-type', contentType)

        // Pass the content type to writeHead so the dev server knows to inject
        // its live reload client (it defers flushing headers for text/html)
