...
describe('extensions', () => {
  ...
  describe('Lookup node module paths', () => {
    it('should lookup asset paths as node module paths', () => {
      expect(extensions.getNodeModulePaths('assets')).toEqual([path.join(
        'govuk-frontend', 'govuk', 'assets'
      )])
    })
    it('should not allow traversing the file system', () => {
      mockExtensionConfig('govuk-frontend', { assets: ['/abc/../../../../../def'] })
      expect(extensions.getNodeModulePaths('assets')).toEqual([path.join(
        'govuk-frontend', 'abc', 'def'
      )])
    })
    it('should show installed extensions asset paths as node module paths', () => {
      delete appConfig.baseExtensions
      mockExtensionConfig('another-frontend', {
        assets: ['/abc', '/def']
      })
      ...
      expect(extensions.getNodeModulePaths('assets')).toEqual([
        path.join('govuk-frontend', 'govuk', 'assets'),
        path.join('another-frontend', 'abc'),
        path.join('another-frontend', 'def'),
        path.join('hmrc-frontend', 'ghi'),
        path.join('hmrc-frontend', 'jkl')
      ])
    })
    it('should follow strict alphabetical order when no base extensions used', () => {
      appConfig.baseExtensions = []
