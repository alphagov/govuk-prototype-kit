...
const getNodeModulePath = config => {
  throwIfBadFilepath(config)
  return path.join(config.packageName, config.item.split('/').filter(filterOutParentAndEmpty).join(path.sep))
}
...
const self = module.exports = {
  ...
  getNodeModulePaths: type => getList(type).map(getNodeModulePath),
