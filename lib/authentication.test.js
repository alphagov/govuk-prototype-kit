  ...
  describe('when it runs in production', () => {
    ...
    describe('server with password set', () => {
      ...
      describe('when a user is not authenticated', () => {
        ...
        it('should be able to authenticate with an auth token', () => {
          testScope.appConfig.authToken = 'abcdef-ghi-jkl'
          testScope.appConfig.password = 'the-password'
          authentication()({
            originalUrl: '/abc/def?theQuery=params-here-will-be-ignored',
            cookies: unauthenticatedCookie,
            query: {
              abc: 'def',
              authToken: testScope.appConfig.authToken,
              ghi: 'jkl'
            }
          }, res, next)

          expect(res.redirect).toHaveBeenCalledWith('/abc/def?abc=def&ghi=jkl')
          expect(res.cookie).toHaveBeenCalled()
          assume(next).not.toHaveBeenCalled()
        })
      })
