...
describe('authentication', () => {
  ...
  describe('when it runs in production', () => {
    ...
    describe('server with no password set', () => {
      ...
      it('should return a user friendly error to the browser', () => {
        ...
        expect(errorDisplayedToUser).toBe('<h1>Error:</h1><p>Password not set. <a href="https://prototype-kit.service.gov.uk/publishing/#setting-a-password">See guidance for setting a password</a>.</p>')
      })
    })
  ...
  describe('authentication is off', () => {
    ...
    it('should progress to the next middleware', () => {
      ...
      assume(console.error).not.toHaveBeenCalled()
      assume(res.send).not.toHaveBeenCalled()
      expect(next).toHaveBeenCalled()
    })
