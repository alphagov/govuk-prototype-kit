...
const { recursiveDirectoryContentsSync, startTimer, getTimeSinceTimerInMilliseconds} = require('../utils')
...
const NunjucksLoader = nunjucks.Loader.extend({
  ...
  init: function (appViews) {
    ...
    const timeSinceTimerInMilliseconds = getTimeSinceTimerInMilliseconds(timer);
    this.performanceEndOfInit = timeSinceTimerInMilliseconds
    console.log('End of init', timeSinceTimerInMilliseconds)
    setInterval(() => {
      const length = this.performanceArr.length;
      let totalTime = 0
      this.performanceArr.forEach(x => totalTime += x)
      console.log('average load time is', totalTime / length)
    }, 5000)
  },
  ...
  getSource: function (name) {
    ...
    if (found) {
      ...
      // }
      const timeSinceTimerInMilliseconds = getTimeSinceTimerInMilliseconds(timer);
      this.performanceArr.push(timeSinceTimerInMilliseconds)
      return result
    }

    const timeSinceTimerInMilliseconds = getTimeSinceTimerInMilliseconds(timer);
    console.log('End of nunjucks getSource (failure)', timeSinceTimerInMilliseconds)
    this.performanceArr.push(timeSinceTimerInMilliseconds)
