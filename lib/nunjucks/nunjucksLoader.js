...
const NunjucksLoader = nunjucks.Loader.extend({
  ...
  getSource: function (name, callback) {
    ...
    const absolutePathsToTry = this.appViews.map(appViewDir => filePathsToTry.map(x => path.join(appViewDir, x))).flat()
    ...
    Promise.all(absolutePathsToTry.map(pathToTry => fse.exists(pathToTry).then(exists => ({
      ...
      .then(results => {
        ...
        if (!firstResultThatExists) {
          console.log('none of these exist:', results.map(x => x.absolutePath))
