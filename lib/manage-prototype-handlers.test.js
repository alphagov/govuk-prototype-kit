...
const { stopPackageTracker } = require('./utils/packages')
...
jest.mock('./plugins/plugins', () => {
  return {
    ...
    preparePackageNameForDisplay: jest.fn().mockImplementation((name) => {
      return ({ name })
    })
  }
})
...
describe('manage-prototype-handlers', () => {
  ...
  describe('plugins handlers', () => {
    ...
    beforeEach(() => {
      ...
      projectPackage.dependencies = { [packageName]: latestVersion }
      const versions = {}
