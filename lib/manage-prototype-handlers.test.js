...
import nunjucksConfiguration from "./nunjucks/nunjucksConfiguration.js";
import config from "./config.js";
import utils from "./utils/index.js";
import exec from "./exec.js";
import plugins from "./plugins/plugins.js";
import { setKitRestarted, getPasswordHandler, getClearDataHandler, getHomeHandler, postClearDataHandler, postPasswordHandler, developmentOnlyMiddleware, getTemplatesHandler, getTemplatesViewHandler, getTemplatesInstallHandler, postTemplatesInstallHandler, getTemplatesPostInstallHandler, getPluginsHandler, postPluginsStatusHandler, postPluginsModeMiddleware, getPluginsModeHandler, postPluginsModeHandler } from "./manage-prototype-handlers.js";
...
describe('manage-prototype-handlers', () => {
    ...
    it('getClearDataHandler', () => {
        ...
        expect(res.render).toHaveBeenCalledWith('views/manage-prototype/clear-data.njk');
    });
    it('postClearDataHandler', () => {
        ...
        expect(res.render).toHaveBeenCalledWith('views/manage-prototype/clear-data-success.njk');
    });
    it('getPasswordHandler', () => {
        req.query.returnUrl = '/';
        getPasswordHandler(req, res);
        expect(res.render).toHaveBeenCalledWith('views/manage-prototype/password.njk', { error: undefined, returnURL: '/' });
    });
    describe('postPasswordHandler', () => {
        ...
        it('incorrect password', async () => {
            ...
            expect(res.redirect).toHaveBeenCalledWith('/manage-prototype/password?error=wrong-password&returnURL=%2F');
        });
    });
    it('getHomeHandler', async () => {
        await getHomeHandler(req, res);
        expect(res.render).toHaveBeenCalledWith('views/manage-prototype/index.njk', expect.objectContaining({ currentSection: 'Home' }));
    });
    describe('developmentOnlyMiddleware', () => {
        it('in production', () => {
            developmentOnlyMiddleware(req, res, next);
            expect(res.render).toHaveBeenCalledWith('views/manage-prototype/manage-prototype-not-available.njk');
        });
        it('in development', () => {
    ...
    describe('templates handlers', () => {
        ...
        it('getTemplatesHandler', async () => {
            ...
            expect(res.render).toHaveBeenCalledWith('views/manage-prototype/templates.njk', expect.objectContaining({
        ...
        describe('getTemplatesInstallHandler', () => {
            describe('template found', () => {
                ...
                async function testGetTemplatesInstallHandler(error) {
                    ...
                    expect(res.status).not.toHaveBeenCalled();
                    expect(res.render).toHaveBeenCalledWith('views/manage-prototype/template-install.njk', expect.objectContaining({
                        currentSection: 'Templates',
            ...
            describe('postTemplatesInstallHandler', () => {
                describe('chosen url success', () => {
                    ...
                    it('where chosen url starts with a forward slash', async () => {
                        ...
                        expect(res.redirect).toHaveBeenCalledWith(`/manage-prototype/templates/post-install?chosen-url=${encodeURIComponent(chosenUrl)}`);
                    });
                    it('where chosen url does not start with a forward slash', async () => {
                        req.body['chosen-url'] = 'no-forward-slash';
                        await postTemplatesInstallHandler(req, res);
                        expect(res.redirect).toHaveBeenCalledWith(`/manage-prototype/templates/post-install?chosen-url=${encodeURIComponent('/no-forward-slash')}`);
                    });
                });
                describe('chosen url failures', () => {
                    const testPostTemplatesInstallHandler = ([errorType, chosenUrl]) => {
                        it(`error type is ${errorType} when chosen url is "${chosenUrl}"`, async () => {
                            req.body['chosen-url'] = chosenUrl;
                            await postTemplatesInstallHandler(req, res);
                            expect(res.redirect).toHaveBeenCalledWith(`${req.originalUrl}?package=${packageName}&template=${encodedTemplatePath}&chosen-url=${encodeURIComponent(chosenUrl)}&errorType=${errorType}`);
                        });
                    };
        ...
        it('getTemplatesPostInstallHandler', async () => {
            ...
            expect(res.render).toHaveBeenCalledWith('views/manage-prototype/template-post-install.njk', expect.objectContaining({
                currentSection: 'Templates',
    ...
    describe('plugins handlers', () => {
        ...
        it('getPluginsHandler', async () => {
            ...
            expect(res.render).toHaveBeenCalledWith('views/manage-prototype/plugins.njk', expect.objectContaining({
        ...
        it('getPluginsModeHandler', async () => {
            ...
            expect(res.render).toHaveBeenCalledWith('views/manage-prototype/plugin-install-or-uninstall.njk', expect.objectContaining({
        ...
        describe('postPluginsModeHandler', () => {
            ...
            it('processing', async () => {
                ...
                expect(exec.exec).toHaveBeenCalledWith(availablePlugin.installCommand, { cwd: projectDir });
                expect(res.json).toHaveBeenCalledWith(expect.objectContaining({
                    status: 'processing'
                }));
            });
