...
async function lookupPackageInfo (packageName, version) {
  const packageInfo = packagesCache[packageName]
  if (!packageInfo) {
    return refreshPackageInfo(packageName, version)
  }
  const packageJsonPath = path.join(projectDir, 'node_modules', packageName, 'package.json')
  if (packageInfo.installed && !(await fse.pathExists(packageJsonPath))) {
    return refreshPackageInfo(packageName, version)
  }
  return packageInfo
}
...
async function getCache () {
  ...
  return Promise.all(Object.keys(packagesCache).map(lookupPackageInfo))
}
...
async function getDependencyPackages (packageName, version) {
  await getCache() // make sure cache is updated
