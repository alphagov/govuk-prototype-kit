...
const {getPackageVersionFromPackageJson, getPackageVersionFromPackageJsonSync} = require('../../bin/utils')
...
const getPluginVersions = () => Object.fromEntries(listInstalledPlugins().map(plugin => [plugin, getPackageVersionFromPackageJsonSync(path.join(projectDir, 'node_modules', plugin, 'package.json'))]))
...
function getAppConfig(additionalConfig) {
  const versions = self.getPluginVersions()
  return {
...
const getConstants = () => {
  ...
  getList('constants').forEach(({packageName, item: {name, value}}) => {
    output[name] = value
  })
