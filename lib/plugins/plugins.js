...
const getPluginConfigForPackageName = packageName => {
  const fileToLoad = [
    getPathFromProjectRoot('node_modules', packageName, 'nowprototypeit.config.json'),
    getPathFromProjectRoot('node_modules', packageName, 'govuk-prototype-kit.config.json')
  ].find(path => fs.existsSync(path))
  if (fileToLoad) {
    return readJsonFile(fileToLoad)
  }
}
...
function getCurrentPlugins () {
  ...
  const dependencies = pkg?.dependencies || {}
  const result = Object.keys(dependencies).filter((dependency) => fse.pathExistsSync(getPluginConfigForPackageName(dependency)))
