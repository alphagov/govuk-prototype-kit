...
import { projectDir, packageDir, shadowNunjucksDir } from "../utils/paths.js";
...
function getPluginsByType() {
    ...
    const result = getPackageNamesInOrder()
        .reduce((accum, packageName) => Object.assign({}, accum, objectMap(getPluginConfig(packageName), (listOfItemsForType, type) => (accum[type] || [])
...
function getFileSystemPath({ packageName, item }) {
    ...
    return getPathFromProjectRoot('node_modules', packageName, item.split('/').filter(filterOutParentAndEmpty).join(path.sep));
}
