...
function validateConfigKeys(pluginConfig) {
  ...
  const configContents = JSON.parse(fse.readFileSync(pluginConfig).toString())
  const knownKeys = getKnownKeys()
  const keysToValidate = Object.keys(configContents);
  ...
  // invalidKeys.map(key, value => {console.log(value)})
  if (invalidKeys) {
    ...
    if (pluginName != null) {
        ...
        fse.exists(pathToPlugin).then(exists => {
          if (exists) {
            ...
            fse.exists(pluginConfig). then(exists => {
              if (exists) {
