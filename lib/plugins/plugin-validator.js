    ...
    pluginConfig[key].forEach((pathToValidate, index) => {
      try {
        if (pathToValidate[0] === '/') {
          ...
          fse.existsSync(pathToValidate) ? console.log('') : console.log(`In section ${key}, the path "${pathToValidate}" does not exist`)
          ...
          fse.existsSync(templatePathToValidate) ? console.log('') : console.log(`In section ${key}, the path "${templatePathToValidate}" does not exist`)
        } else {
          const invalidPath = (key === 'templates') ? pathToValidate.path : pathToValidate
          console.log(`In section ${key}, the path "${invalidPath}" does not start with a '/'`)
        }
      } catch (e) {
        console.log(e)
      }
