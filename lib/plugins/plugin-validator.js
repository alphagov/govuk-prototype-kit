...
function checkNunjucksMacroExists (executionPath, nunjucksFileName, nunjucksPaths) {
  ...
  if (nunjucksPaths !== undefined) {
    for (const nunjucksPath in nunjucksPaths) {
    ...
    checkPathExists(executionPath, nunjucksFileName, 'nunjucksMacros')
  }
    ...
    criteriaConfig.forEach((configEntry, index) => {
      try {
        if (typeof configEntry === 'string' && configEntry[0] === '/') {
          ...
          checkNunjucksMacroExists(executionPath, configEntry.importFrom, criteriaConfig.nunjucksPaths)
        } else {
