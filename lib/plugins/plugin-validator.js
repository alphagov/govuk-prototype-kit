  ...
  await fse.exists(configPath).then(exists => {
    if (exists) {
      ...
      try {
        ...
        errors.push('Your govuk-prototype-kit.config.json file is not a valid json.')
      }

      // If there are no json errors in the config then the rest of the checks can be performed
      if (pluginConfig) {
