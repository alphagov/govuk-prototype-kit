...
function getKnownKeys () {
  ...
  // TODO: Look into known plugin keys as we initially though nunjucksMacros did not exist in the known plugins list
  return knownKeys
}
  ...
  keysToValidate.forEach(key => {
    ...
    criteriaConfig.forEach((pathToValidate, index) => {
      ...
      try {
        ...
        } else if ((key === 'templates' && pathToValidate.path[0] === '/') || (key === 'scripts' && pathToValidate.path !== undefined && pathToValidate.path[0] === '/')) {
        ...
        } else if ((key === 'nunjucksMacros')) { // TODO: Should we be checking for && !pathToValidate.importFrom[0] === '/'
          // Check if template paths are valid differently as the schema for these is different
          absolutePathToValidate = path.join(executionPath, pathToValidate.importFrom)
          const templatePathToValidate = pathToValidate.importFrom
          if (!fse.existsSync(absolutePathToValidate)) errors.push(`In section ${key}, the path '${templatePathToValidate}' does not exist`)
        } else {
