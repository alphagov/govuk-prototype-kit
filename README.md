...
You can test changes to the prototype kit package locally by running `npm run start:dev`. This will create a new prototype from your local prototype kit package in your device's `tmp` directory and automatically run `npm start`.

If you wanted more control over a local prototype generated by your local prototype kit package, you can run the following in an empty directory on your device:

```bash
npx ../govuk-prototype-kit create --version local
```

in the above example, the directory is adjacent to where the local instance of the prototype kit itself is, however the important this is running `npx` against the path to your local prototype kit.
