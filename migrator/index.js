...
import { npmInstall, packageJsonFormat } from "../bin/utils/index.js";
import { preflightChecks, migrateConfig, prepareAppRoutes, prepareSass, deleteUnusedFiles, deleteUnusedDirectories, deleteEmptyDirectories, upgradeIfUnchanged, deleteIfUnchanged, removeOldPatternIncludesFromSassFile, updateUnbrandedLayouts, upgradeIfPossible } from "./migration-steps.js";
...
async function upgradeLayoutIfUnchanged() {
    const results = await upgradeIfUnchanged(['app/views/layout.html'], 'app/views/layouts/main.html', () => deleteIfUnchanged([
...
async function performPreflightChecks() {
    await logger.setup();
    const success = await preflightChecks(filesToIdentifyPreV13Prototype, folderToIdentifyV6Prototype, minimumPrototypeVersionForMigration);
    if (!success) {
...
export { prepareMigration };
export { performPreflightChecks as preflightChecks };
export { migrate };
export default {
    prepareMigration,
