...
import { searchAndReplaceFiles } from "../lib/utils/index.js";
import { appDir, projectDir, packageDir } from "../lib/utils/paths.js";
import config from "../lib/config.js";
import logger from "./logger.js";
import { addReporter, reportSuccess, reportFailure } from "./reporter.js";
import { getFileAsLines, deleteFile, replaceStartOfFile, removeLineFromFile, deleteDirectory, matchAgainstOldVersions, copyFileFromStarter, verboseLog, handleNotFound, deleteDirectoryIfEmpty, writeFileLinesToFile } from "./file-helpers.js";
...
async function preflightChecks(filesToCheck, v6Folder, minimumVersion) {
    ...
    const checksPass = (!await fse.pathExists(v6Folder) &&
...
async function updateUnbrandedLayouts(dir) {
    const results = await searchAndReplaceFiles(path.join(projectDir, dir), '"layout_unbranded.html"', '"govuk-prototype-kit/layouts/unbranded.html"', ['.html', '.njk']);
    return results.flat();
}
export { getOldConfig };
export { preflightChecks };
export { migrateConfig };
export { prepareAppRoutes };
export { prepareSass };
export { removeOldPatternIncludesFromSassFile };
export { deleteUnusedFiles };
export { deleteUnusedDirectories };
export { deleteEmptyDirectories };
export { deleteIfUnchanged };
export { upgradeIfUnchanged };
export { updateUnbrandedLayouts };
export { upgradeIfPossible };
export default {
    getOldConfig,
    preflightChecks,
    migrateConfig,
