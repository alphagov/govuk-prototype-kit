...
async function getPluginListItems (self) {
  return await Promise.all(await self.driver.findElements(By.className('nowprototypeit-manage-prototype-plugin-list-plugin-list__item')))
}
...
Then('The {string} plugin should be tagged as {string}', async function (pluginName, tag) {
  expect((await getTagsForPlugin(this, pluginName)).map(x => x.toUpperCase())).toContain(tag.toUpperCase())
})
...
Given('I uninstall the {string} plugin', async function (pluginNameOrRef) {
  ...
  const uninstallButton = await this.driver.findElement(By.className('nowprototypeit-plugin-uninstall-button'))
...
Given('I update the {string} plugin', async function (pluginNameOrRef) {
  // TODO: Support names as well as refs
  await this.visit(`/manage-prototype/plugin/${encodeURIComponent(pluginNameOrRef)}`, lookForH1(this))
  const updateButton = await this.driver.findElement(By.className('nowprototypeit-plugin-update-button'))
  updateButton.click()
})
