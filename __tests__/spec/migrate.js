...
import { normaliseLineEndings } from "../../migrator/file-helpers.js";
...
describe('migrate test prototype', () => {
    beforeAll(async () => {
        ...
        await exec(`"${process.execPath}" ${cliPath} migrate --version local ${projectDirectory}`, { cwd: projectDirectory, env: process.env, stdio: 'inherit' });
    }, 240000);
    ...
    it('filters.js should be overwritten', () => {
        ...
        expect(filtersFileContents).toEqual('const govukPrototypeKit = require(\'govuk-prototype-kit\')\n' +
...
`);
    ...
    it('unbranded-test.html should replace unbranded extend', () => {
        ...
        expect(unbrandedFileContents).toEqual('{% extends "govuk-prototype-kit/layouts/unbranded.html" %}\n' +
            '{% block pageScripts %}\n' +
            '  <script>\n' +
    ...
    it('migrate.log does not contain user home directory', () => {
        ...
        expect(migrateLog.split('\n').filter(line => line.includes(testDirectory) && !line.startsWith('argv:') && !line.startsWith('cwd:'))).toEqual([]);
    });
});
