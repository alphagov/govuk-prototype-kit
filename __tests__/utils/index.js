 ...
 * @returns {void}
 */
async function mkPrototype(prototypePath, { kitPath, overwrite = false, allowTracking = undefined, npmInstallLinks = undefined, commandLineParameters = '' } = {}) {
    ...
    try {
        ...
        await exec(`"${process.execPath}" bin/cli create --version local ${commandLineParameters} ${prototypePath}`, { cwd: repoDir, env: execEnv, stdio: 'inherit' });
    ...
    if (npmInstallLinks !== undefined) {
        ...
        await exec(`npm config --location=project set install-links=${npmInstallLinks ? 'true' : 'false'}`, { cwd: prototypePath });
    }
}
async function installPlugins(prototypePath, pluginNames) {
    ...
    return exec(`npm install ${pluginNamesProcessed.join(' ')}`, { cwd: prototypePath, env: { ...process.env, env: 'test' }, stdio: 'inherit' });
}
async function startPrototype(prototypePath, nodeEnv = 'development') {
    return exec(nodeEnv === 'production' ? 'npm start' : 'npm run dev', { cwd: prototypePath, env: { USE_AUTH: 'false', USE_HTTPS: 'false', ...process.env, env: 'test' }, stdio: 'inherit' });
}
export { mkdtempSync };
export { mkPrototype };
export { startPrototype };
